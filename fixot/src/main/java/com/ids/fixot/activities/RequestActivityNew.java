...
import com.ids.fixot.Custom.DecimalDigitsInputFilter;
import com.ids.fixot.Custom.InputFilterMinMax;
...
import com.ids.fixot.adapters.RequestTypeSpinnerAdapter;
...
public class RequestActivityNew extends AppCompatActivity implements RequestRecyclerAdapter.RecyclerViewOnItemClickListener, PortoflioStockForwardAdapter.RecyclerViewOnItemClickListener, MarketStatusListener , spItemListener {
    ...
    LinearLayout linearAmountEditText;
    Button btSubmit;
    Double initialAvailableAmount=0.0;
    private String selectedDate="";
    RecyclerView rvRequestTickets;
   ...
   String selectedTypeValue= "";
    ...
    GetCashTransactionRequests getCashTransRequests = new GetCashTransactionRequests();
    SaveChequeRequest saveChequeRequest = new SaveChequeRequest();
    GetClientTypes getClientTypes = new GetClientTypes();
    LinearLayout llTotalStocks;

    Calendar myCalendar;
    DatePickerDialog.OnDateSetListener date;

    private RequestTypeSpinnerAdapter adapterTypes;

    private BroadcastReceiver receiver;
    private boolean started = false, showProgress = false;
    Spinner spInstrumentsTop;
    public RequestActivityNew() {
        LocalUtils.updateConfig(this);
    }

    RequestRecyclerAdapter adapter;
    LinearLayoutManager llm;
    ArrayList<ResponseRequestTickets> allTicketsArray = new ArrayList<>();
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        getClientTypes = new GetClientTypes();
        getClientTypes.executeOnExecutor(MyApplication.threadPoolExecutor);
        findViews();

        getAvailableForWithdrawal = new GetAvailableForWithdrawal();
        getAvailableForWithdrawal.executeOnExecutor(MyApplication.threadPoolExecutor);

        getCashTransRequests = new GetCashTransactionRequests();
        getCashTransRequests.executeOnExecutor(MyApplication.threadPoolExecutor);
        ...
        Actions.initializeToolBar(getString(R.string.portfolio), RequestActivityNew.this);
    ...
    private void findViews() {
        ...
        rvRequestTickets = findViewById(R.id.rvCheckRequests);
        ...
        tvToDate.setOnClickListener(v -> new DatePickerDialog(RequestActivityNew.this, date, myCalendar
        ...
        linearAmountEditText=findViewById(R.id.linearAmountEditText);

        tvLogout = myToolbar.findViewById(R.id.tvLogout);
        tvLogout.setOnClickListener(v -> Actions.logout(RequestActivityNew.this));
...
//    }
    ...
    public void onItemClicked(View v, int position) {
     if(v.getId()==R.id.btCancelRequest){
...
//            arrayOfStocks.add(stockss);//header
            ...
            if (showProgress)
                MyApplication.showDialog(RequestActivityNew.this);
        ...
        protected Void doInBackground(Void... params) {
            ...
            try {
                result = ConnectionRequests.GET(url, RequestActivityNew.this, parameters);
        ...
        protected void onPostExecute(String result) {
            ...
            try {
                ...
                try{
                    ...
                    etAmount.setText(Actions.formatNumber(initialAvailableAmount, Actions.ThreeDecimal));
                ...
                e.printStackTrace();
                Actions.CreateDialog(RequestActivityNew.this, getResources().getString(R.string.error), false, false);
            }
            // setStockAlerts();
    ...
    private void checkAmount(){
        ...
        if(Double.parseDouble(etAmount.getText().toString())>initialAvailableAmount ){
            linearAmountEditText.setBackgroundResource(R.drawable.border_edit_text_wrong);
            etRemaining.setText("0.0");
        }else if(etAmount.getText().toString().contains(".")){
                Log.wtf("substring_amount",etAmount.getText().toString().substring(
                        etAmount.getText().toString().indexOf(".")));
                if(etAmount.getText().toString().substring(
                        etAmount.getText().toString().indexOf(".")
                ).length()>4){
                    linearAmountEditText.setBackgroundResource(R.drawable.border_edit_text_wrong);
                    etRemaining.setText("0.0");
                }else {
                    linearAmountEditText.setBackgroundResource(R.drawable.border_edit_text);
                    etRemaining.setText(
                            Actions.formatNumber((initialAvailableAmount-Double.parseDouble(etAmount.getText().toString())), Actions.ThreeDecimal)
                    )  ;
                }
                    ...
                    etRemaining.setText(
                        ...
                        etRemaining.setText(initialAvailableAmount.toString());
                    }
                }}else {
                 etRemaining.setText(initialAvailableAmount.toString());
            }
    ...
    private void checkBeforeSending(){
        ...
        }else if(linearAmountEditText.getBackground().getConstantState()==getResources().getDrawable(R.drawable.border_edit_text_wrong).getConstantState()){
            Actions.CreateDialog(this,getString(R.string.plz_check_amount),false,false);
        }
    ...
    private class SaveChequeRequest extends AsyncTask<Void, String, String> {
        ...
        protected String doInBackground(Void... params) {
            ...
            try {
                stringer = new JSONStringer()
                        ...
                        .key("TicketDate").value(tvToDate.getText().toString())
                        .key("key").value(MyApplication.mshared.getString(getString(R.string.afterkey), ""))
                        .key("TypeID").value(selectedTypeId + "")
                        .endObject();
                Log.wtf("SaveChequeRequest : url ='" + url + "'", " / JSONStringer = '" + stringer.toString() + "'");
            } catch (JSONException e) {
                e.printStackTrace();
                if (MyApplication.isDebug) {
                    runOnUiThread(new Runnable() {
        ...
        protected void onPostExecute(String result) {
            ...
            try {
                ...
                if (success) {
                   // CreateDialog(RequestActivityNew.this,message);
                    getCashTransRequests = new GetCashTransactionRequests();
                    getCashTransRequests.executeOnExecutor(MyApplication.threadPoolExecutor);
                } else {
                    String error;
                    Actions.CreateDialog(RequestActivityNew.this, message, false, false);
                }
            } catch (JSONException e) {
                e.printStackTrace();
                Actions.CreateDialog(RequestActivityNew.this, getResources().getString(R.string.error), false, false);
    ...
    private void setTypesSpinner(){
        ...
        adapterTypes = new RequestTypeSpinnerAdapter(this, MyApplication.arrayTicketTypes, true);
        spTypes.setAdapter(adapterTypes);
        spTypes.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                try{
                selectedTypeId= MyApplication.arrayTicketTypes.get(position).getCashTicketTypeID();
                selectedTypeValue= MyApplication.arrayTicketTypes.get(position).getDescriptionEn()+"";}catch (Exception e){}
            }
    ...
    private class GetClientTypes extends AsyncTask<Void, String, String> {
        ...
        protected String doInBackground(Void... params) {
            ...
            String result = "";
            MyApplication.arrayTicketTypes.clear();
            String url = MyApplication.link + MyApplication.GetClientTypes.getValue(); // this method uses key after login
            HashMap<String, String> parameters = new HashMap<String, String>();
          ...
          //  parameters.put("key", MyApplication.mshared.getString(getString(R.string.afterkey), ""));
            ...
            try {
                result = ConnectionRequests.GET(url, getApplicationContext(), parameters);
                MyApplication.arrayTicketTypes.addAll(GlobalFunctions.GetClientTypes(result));
            } catch (Exception e) {
                e.printStackTrace();
    ...
    private class GetCashTransactionRequests extends AsyncTask<Void, String, String> {
        ...
        protected String doInBackground(Void... params) {
            ...
            String result = "";
            allTicketsArray.clear();
            String url = MyApplication.link + MyApplication.GetCashTransactionRequests.getValue(); // this method uses key after login
            HashMap<String, String> parameters = new HashMap<String, String>();
            ...
            try {
                result = ConnectionRequests.GET(url, getApplicationContext(), parameters);
                allTicketsArray.addAll(GlobalFunctions.GetRequestTickets(result));
            } catch (Exception e) {
                e.printStackTrace();
    ...
    private void setRequestsRecycler(){
        adapter = new RequestRecyclerAdapter(this, allTicketsArray, RequestActivityNew.this);
        llm = new LinearLayoutManager(RequestActivityNew.this);
        rvRequestTickets.setAdapter(adapter);
        rvRequestTickets.setLayoutManager(llm);
    }
